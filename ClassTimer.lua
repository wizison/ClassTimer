...
local hasPet = enClass == "HUNTER"
	or enClass == "WARLOCK"
	or enClass == "DEATHKNIGHT"
	or enClass == "MAGE"
	or enClass == "SHAMAN" and true
local unlocked = {}
local sticky = {}
...
do
	...
	function OnUpdate(frame)
		...
		if currentTime > endTime then
			if unlocked[frame.unit] then
				unlocked[frame.unit] = nil
...
function ClassTimer:OnInitialize()
		...
		timerargs[k] = {
			...
			args = {
				shown = {
					...
					get = function(_, key)
						return self.db.profile.Abilities[key]
					end,
					set = function(_, key, value)
						self.db.profile.Abilities[key] = value
						validate[key] = value and key or nil
					end,
		...
		args = {
			Add = {
				...
				set = function(_, value)
					self.db.char.Custom[value] = value
	...
	timerargs.AlwaysShown = {
		...
		args = {
			Add = {
				...
				set = function(_, value)
					self.db.char.AlwaysShown[value] = value
					validate[value] = value
				end,
				usage = L["<Spell Name in games locale>"],
			},
			Remove = {
				...
				set = function(_, key)
					self.db.char.AlwaysShown[key] = nil
					validate[key] = nil
				end,
	...
	self.options.args.BarSettings.args.sticky.args.Sticky.args.addSticky = {
		...
		get = function(_, v)
			return self.db.profile.Sticky[v]
		end,
		order = 5,
		set = function(_, v, u)
			self.db.profile.Sticky[v] = u
		end,
		values = validate,
	}
	...
	self:RegisterChatCommand("ClassTimer", function()
		LibStub("AceConfigDialog-3.0"):Open("ClassTimer")
	end)
...
do
	...
	function ClassTimer:GetBuffs(unit, db)
		...
		if db.buffs then
			...
			while true do
				...
				if
					duration and (self.db.profile.Abilities[name] or self.db.char.Custom[name]) and isMine
					or self.db.char.AlwaysShown[name]
				then
					local t = new()
		...
		if db.debuffs then
			...
			while true do
				...
				if
					duration
						and duration > 0
						and (self.db.profile.Abilities[name] or self.db.char.Custom[name])
						and isMine
					or self.db.char.AlwaysShown[name]
				then
					local t = new()
	...
	function ClassTimer:UpdateUnitBars(unit)
		...
		if unlocked[unit] then
			return
		end
		if self.db.profile.Group[unit] then
			unit = self.db.profile.AllInOneOwner
		end
		if called then
			return
		end
		...
		if db.enable then
			...
			if self.db.profile.Group[unit] then
				for k, v in pairs(self.db.profile.Group) do
					if v then
						if k ~= "focus" then
							if not UnitIsUnit(k, unit) then
								self:GetBuffs(k, db)
							end
						else
							if not UnitIsUnit(k, unit) and not UnitIsUnit(k, "target") then
								self:GetBuffs(k, db)
							end
						end
					end
				...
				if not db.showIcons then
					if bar.alwaysshown then
						bar:SetStatusBarColor(
							unpack(bar.isbuff and db.alwaysshownbuffcolor or db.alwaysshowndebuffcolor)
						)
